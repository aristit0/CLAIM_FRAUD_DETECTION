...
// Master Data (sama dengan existing)
const ICD10_DATA = [
...
const AUTOFILL_TEMPLATES = {
  ...
  normalGastritis: {
    ...
    data: {
      ...
      drugs: [
        { code: "KFA004", cost: 35000 },
        { code: "KFA023", cost: 25000 },
      ],
  ...
  fraudMismatch: {
    ...
    data: {
      ...
      diagnosis_secondary: "",
      procedures: [
        { code: "45.13", cost: 750000 },
        { code: "93.05", cost: 250000 },
      ],
      drugs: [
        { code: "KFA035", cost: 150000 },
        { code: "KFA007", cost: 85000 },
      ],
      vitamins: [
        { name: "Vitamin B Complex", cost: 75000 },
        { name: "Vitamin C 500 mg", cost: 50000 },
        { name: "Zinc 20 mg", cost: 35000 },
      ],
    }
  },
  fraudCostAnomaly: {
    ...
    data: {
      ...
      diagnosis_secondary: "",
      procedures: [
        { code: "89.02", cost: 450000 },
        { code: "89.14", cost: 550000 },
        { code: "96.04", cost: 650000 },
      ],
      drugs: [
        { code: "KFA001", cost: 125000 },
        { code: "KFA009", cost: 95000 },
        { code: "KFA026", cost: 85000 },
        { code: "KFA031", cost: 75000 },
      ],
      vitamins: [
        { name: "Multivitamin Adult", cost: 150000 },
        { name: "Vitamin C 500 mg", cost: 100000 },
        { name: "Vitamin B1 100 mg", cost: 80000 },
      ],
    }
  },
}
...
export default function SubmitClaimPage() {
  ...
  const applyAutofill = (templateKey) => {
    ...
    setFormData({
      patient_name: template.data.patient_name,
      patient_nik: template.data.patient_nik,
      patient_dob: template.data.patient_dob,
      patient_gender: template.data.patient_gender,
      patient_phone: template.data.patient_phone,
      patient_address: template.data.patient_address,
      visit_date: template.data.visit_date,
      visit_type: template.data.visit_type,
      doctor_name: template.data.doctor_name,
      department: template.data.department,
      diagnosis_primary: template.data.diagnosis_primary,
      diagnosis_secondary: template.data.diagnosis_secondary || '',
    })
    ...
    toast({
      ...
      variant: template.type === 'fraud' ? 'destructive' : 'success',
    })
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Button
            type="button"
            variant={showQuickFill ? "default" : "outline"}
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${template.type === 'normal' 
                          ...
                          : 'bg-red-500/10 border border-red-500/20 hover:border-red-500/40 hover:bg-red-500/15'
                        }
                      `}
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`text-sm font-medium ${template.type === 'normal' ? 'text-emerald-400' : 'text-red-400'}`}>
                          {template.label}
                        </span>
                      </div>
          ...
          <Card className="border-slate-800 bg-slate-900/50 backdrop-blur">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <Label>Gender *</Label>
                  <Select value={formData.patient_gender} onValueChange={(v) => handleInputChange('patient_gender', v)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.visit_type} onValueChange={(v) => handleInputChange('visit_type', v)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.diagnosis_primary} onValueChange={(v) => handleInputChange('diagnosis_primary', v)}>
                    <SelectTrigger>
          ...
          <Card className="border-slate-800 bg-slate-900/50 backdrop-blur">
            <CardHeader>
              <CardTitle className="text-lg flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => addRow('procedure')} className="gap-1">
                ...
                {procedures.map((proc, idx) => (
                  ...
                  >
                    <div className="flex-1 space-y-1">
                      ...
                      <Select value={proc.code} onValueChange={(v) => updateRow('procedure', idx, 'code', v)}>
                        <SelectTrigger>
                    ...
                    <div className="w-40 space-y-1">
                      <Label className="text-xs text-slate-500">Cost (Rp)</Label>
                      <Input
                        type="number"
                        value={proc.cost}
                        onChange={(e) => updateRow('procedure', idx, 'cost', e.target.value)}
                        placeholder="0"
                      />
                    </div>
                    {procedures.length > 1 && (
                      <Button type="button" variant="ghost" size="icon" onClick={() => removeRow('procedure', idx)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10">
          ...
          <Card className="border-slate-800 bg-slate-900/50 backdrop-blur">
            <CardHeader>
              <CardTitle className="text-lg flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => addRow('drug')} className="gap-1">
                  <Plus className="w-4 h-4" /> Add
                </Button>
              </CardTitle>
            </CardHeader>
                ...
                {drugs.map((drug, idx) => (
                  ...
                  >
                    <div className="flex-1 space-y-1">
                      ...
                      <Select value={drug.code} onValueChange={(v) => updateRow('drug', idx, 'code', v)}>
                        <SelectTrigger>
                    ...
                    <div className="w-40 space-y-1">
                      ...
                      <Input
                        type="number"
                        value={drug.cost}
                        onChange={(e) => updateRow('drug', idx, 'cost', e.target.value)}
                        placeholder="0"
                      />
                    </div>
                    {drugs.length > 1 && (
                      <Button type="button" variant="ghost" size="icon" onClick={() => removeRow('drug', idx)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10">
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    )}
                  </motion.div>
          ...
          <Card className="border-slate-800 bg-slate-900/50 backdrop-blur">
            <CardHeader>
              <CardTitle className="text-lg flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => addRow('vitamin')} className="gap-1">
                  <Plus className="w-4 h-4" /> Add
                </Button>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {vitamins.map((vit, idx) => (
                  ...
                  >
                    <div className="flex-1 space-y-1">
                      <Label className="text-xs text-slate-500">Vitamin</Label>
                      <Select value={vit.name} onValueChange={(v) => updateRow('vitamin', idx, 'name', v)}>
                        <SelectTrigger>
                          <SelectValue placeholder="Select vitamin" />
                        </SelectTrigger>
                        <SelectContent>
                          {VITAMINS_DATA.map(v => (
                    ...
                    <div className="w-40 space-y-1">
                      ...
                      <Input
                        type="number"
                        value={vit.cost}
                        onChange={(e) => updateRow('vitamin', idx, 'cost', e.target.value)}
                        placeholder="0"
                      />
                    </div>
                    {vitamins.length > 1 && (
                      <Button type="button" variant="ghost" size="icon" onClick={() => removeRow('vitamin', idx)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10">
                        <Trash2 className="w-4 h-4" />
                      </Button>
                    )}
                  </motion.div>
